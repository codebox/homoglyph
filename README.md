...
I have tried to compile a list of all the homoglyphs I could find, and to make the list useful by processing it in various ways to make it easier to use in software. The list of homoglyphs I used is based on [the one that appears on the Unicode Consortium website](https://www.unicode.org/Public/security/latest/confusables.txt) however this list, although long, was incomplete, so I added some further pairs found thanks to homoglyphs.net (which unfortunately is no longer online).
...
* Here is a [file listing multi-character homoglyphs](https://github.com/codebox/homoglyph/blob/master/raw_data/multi_char_codes.txt) - single characters that resemble a sequence of two or more others (for example 'ﬃ' looks like 'ffi', and 'm' looks like 'rn'), again as hexadecimal codepoint values
